    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        ...
        <div class="layui-inline">
          ...
          <div class="layui-input-inline">
            <input type="text" name="shifouzhuangxiu" id="shifouzhuangxiu" placeholder="是否装修" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline">
        ...
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('fangyuanxinxi','新增')" @click="jump('../fangyuanxinxi/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="box">
        <ul class="clearfix">
          <li v-for="(item,index) in dataList" v-bind:key="index">
            <a :href="'../fangyuanxinxi/detail.jsp?id='+item.id">
              <figure>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#fangdongxingming').val()) {
          param['fangdongxingming'] = jquery('#fangdongxingming').val() ? '%' + jquery('#fangdongxingming').val() + '%' : '';
        }  
        if (jquery('#shifouzhuangxiu').val()) {
          param['shifouzhuangxiu'] = jquery('#shifouzhuangxiu').val() ? '%' + jquery('#shifouzhuangxiu').val() + '%' : '';
        }  
        if (jquery('#chaoxiang').val()) {
          param['chaoxiang'] = jquery('#chaoxiang').val() ? '%' + jquery('#chaoxiang').val() + '%' : '';
        }  
        if (jquery('#geju').val()) {
          param['geju'] = jquery('#geju').val() ? '%' + jquery('#geju').val() + '%' : '';
        }  
        // 获取列表数据
