      ...
      <form class="layui-form" style="margin: 10px 0;">
        <div class="layui-inline">
          ...
          <div class="layui-input-inline">
            <input type="text" name="shebeimingcheng" id="shebeimingcheng" placeholder="设备名称" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline">
        ...
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('baoxiuxinxi','新增')" @click="jump('../baoxiuxinxi/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="box">
        <ul class="clearfix">
          <li v-for="(item,index) in dataList" v-bind:key="index">
            <a :href="'../baoxiuxinxi/detail.jsp?id='+item.id">
              <figure>
              </figure>
              <div class="div1">
                <b></b>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#shebeimingcheng').val()) {
          param['shebeimingcheng'] = jquery('#shebeimingcheng').val() ? '%' + jquery('#shebeimingcheng').val() + '%' : '';
        }  
        if (jquery('#yonghuxingming').val()) {
          param['yonghuxingming'] = jquery('#yonghuxingming').val() ? '%' + jquery('#yonghuxingming').val() + '%' : '';
        }  
        if (jquery('#fangdongzhanghao').val()) {
          param['fangdongzhanghao'] = jquery('#fangdongzhanghao').val() ? '%' + jquery('#fangdongzhanghao').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('baoxiuxinxi/list', 'get', param, function(res) {
